...
| animation | `TweenInfo`, real `Tween` semantics | physics spring | `TweenService` directly |
| animation retarget | restarts from current value at zero velocity | carries velocity | `Tween:Cancel` then a new tween |
...
you want one value fanned out to several instances, or when you want
`TweenInfo` semantics with a handle you can `Await` and cancel.

Pick **Vide** when you want fine-grained reactivity, derived values, and
physics animation that survives interruption.
